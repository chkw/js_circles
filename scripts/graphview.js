 ...
 * @param {Object} config   Has attributes for 'height' and 'width'.
 */
jQuery.fn.graphViewer = function(config) {
    ...
    this.render = function() {
        ...
        circle.on('mouseover', function(d) {
            ...
            tooltip_div.html("<div>" + d.label + "</div><div>" + d.attr.type + "</div>").style("left", (d3.event.pageX + 40) + "px").style("top", (d3.event.pageY - 35) + "px");
        ...
        /**
         *This function will be triggered by "tick" event.  D3 will use it to position graph nodes.
         */
        function tick() {
        ...
        g.append('svg:circle').attr('class', 'node').attr('r', 8).style('stroke', 'black').attr('fill', function(d) {
            return colors(d['attr']['type'])
        });

        path.enter().append('svg:path').attr('class', 'link')

        // init D3 force layout

        var force = d3.layout.force().nodes(nodes).links(links).size([width, height]).linkDistance(25).charge(-50).on('tick', tick)
