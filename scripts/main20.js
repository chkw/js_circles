...
(function(cmGraph) {"use strict";
    ...
    cmGraph.build = function(config) {
        ...
        // add targetting drugs data
        if (config.hasOwnProperty("targettingDrugsData")) {
            var targettingDrugsData = config["targettingDrugsData"];
            graphDataObj.readClinicalEventsSummaryDocs(targettingDrugsData);
        }
        ...
        if (utils.hasOwnProperty(config, "patientSamples")) {
            var dataArray = config["patientSamples"];
    ...
    cmGraph.renderGraph = function(svg, force, graph, cmg, circleDataLoaded) {
        ...
        nodeSelection.append(function(d) {
            ...
            if ((type != "drug") && (circleDataLoaded ) && (nodeNames.indexOf(nodeName) >= 0)) {
                ...
                newElement.setAttributeNS(null, 'class', 'sbgn');
                newElement.setAttributeNS(null, 'r', d3_config['nodeRadius']);
                newElement.setAttributeNS(null, 'opacity', opacityVal);
                newElement.setAttributeNS(null, 'stroke', 'black');
                return newElement;
